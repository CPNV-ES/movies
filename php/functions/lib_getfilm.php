...
function getFilms(&$films)
{
	...
	foreach ($paths as $path) {
		...
		foreach ($name as $row) {
			$matches = array();
			preg_match('/^(\[.*\])?[_\.\s]?(([a-zA-Z0-9����]{1,})([_\.\s]([A-Z]?([a-z����]{1,}|[I]{1,})?|(?!(19|20|21)[0-9]{2})[0-9]{1,}|\%\![0-9]{1,}))*)([_\.\s]((\(?([0-9]){4}\)?|[A-Z]{4,}|(([sSeE](aison|eason|pisode)?)[_\s]?[0-9]{1,})[\s-]*(([sSeE](aison|eason|pisode)?)[_\s]?[0-9]{1,})?).*))?\.(avi|mp4|mov|mpg|mpa|wma)$/', $row, $matches);
			if(isset($matches[2])) {
...
function getFilmsByPaths(&$films, $paths)
{
	...
	foreach ($paths as $path) {
		...
		foreach ($name as $row) {
			$matches = array();
			preg_match('/^(\[.*\])?[_\.\s]?(([a-zA-Z0-9éèàô&]{1,})([_\.\s]([A-Z]?([a-zéèàô&]{1,}|[I]{1,})?|(?!(19|20|21)[0-9]{2})[0-9]{1,}|\%\![0-9]{1,}))*)([_\.\s]((\(?([0-9]){4}\)?|[A-Z]{4,}|(([sSeE](aison|eason|pisode)?)[_\s]?[0-9]{1,})[\s-]*(([sSeE](aison|eason|pisode)?)[_\s]?[0-9]{1,})?).*))?\.(avi|mp4|mov|mpg|mpa|wma)$/', $row, $matches);
			if (isset($matches[2])) {
				$tmp = preg_replace('/[_\.]/', ' ', $matches[2]);
				$tmp = strtolower($tmp);
				if (!array_search($tmp, $films)) {
